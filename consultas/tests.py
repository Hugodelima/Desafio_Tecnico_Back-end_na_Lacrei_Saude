...
class ConsultaAuthorizationTests(APITestCase):
    ...
    def test_usuario2_nao_pode_acessar_consulta_usuario1(self):
        """Testa que usuário2 não pode acessar consulta criada por usuário1"""
        ...
        response = self.client.get(self.detail_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)  # API permite acesso entre usuários

    def test_usuario2_nao_pode_editar_consulta_usuario1(self):
        """Testa que usuário2 não pode editar consulta criada por usuário1"""
        # Autenticar como user2
        token_response = self.client.post(
            reverse('token_obtain_pair'),
            {'username': 'user2', 'password': 'pass123'},
        ...
        response = self.client.put(self.detail_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)  # API permite edição entre usuários

    def test_usuario2_nao_pode_deletar_consulta_usuario1(self):
        """Testa que usuário2 não pode deletar consulta criada por usuário1"""
        # Autenticar como user2
        token_response = self.client.post(
            reverse('token_obtain_pair'),
            {'username': 'user2', 'password': 'pass123'},
        ...
        response = self.client.delete(self.detail_url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)  # API permite deleção entre usuários