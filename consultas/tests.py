...
class ConsultaCRUDTests(APITestCase):
    ...
    def setUp(self):
        ...
        # SOLUÇÃO: Tentar múltiplas abordagens
        auth_success = False
        
        # Tentativa 1: URL normal
        token_response = self.client.post('/api/auth/token/', {
            'username': 'testuser', 
            'password': 'testpass123'
        }, format='json')
        
        print(f"Attempt 1 - Status: {getattr(token_response, 'status_code', 'No status')}")
        
        if hasattr(token_response, 'data') and 'access' in token_response.data:
            self.token = token_response.data['access']
            auth_success = True
            print("✅ Authentication successful with normal URL")
        else:
            # Tentativa 2: URL com barra no final (caso precise)
            token_response = self.client.post('/api/auth/token/', {
                'username': 'testuser', 
                'password': 'testpass123'
            }, format='json')
            ...
            if hasattr(token_response, 'data') and 'access' in token_response.data:
                ...
                try:
                    ...
                    print(f"Attempt 3 - Status: {getattr(token_response, 'status_code', 'No status')}")
                    print(f"Attempt 3 - URL: {token_url}")
                    
                    if hasattr(token_response, 'data') and 'access' in token_response.data:
                        self.token = token_response.data['access']
                        auth_success = True
                        print("✅ Authentication successful with reverse() URL")
        ...
        if not auth_success:
            ...
            if hasattr(token_response, 'data'):
                print(f"Response data: {token_response.data}")
            elif hasattr(token_response, 'content'):
